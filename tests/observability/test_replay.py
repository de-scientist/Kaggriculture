...
def test_record_creates_replay_record() -> None:
    ...
    trace = Trace(
        correlation_id="c-1", decision_id="d-5", step=5, day=1, player=0, strategy="baseline"
    )
    rec = store.record(
        turn=5,
        day=1,
...
def test_find_by_decision_id() -> None:
    ...
    store = reset_replay_store()
    trace = Trace(
        correlation_id="c-1", decision_id="d-1", step=1, day=0, player=0, strategy="baseline"
    )
    store.record(
        turn=1,
        day=0,
